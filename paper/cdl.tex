...
\STATE{Initialize $A \leftarrow 0, \quad Z \leftarrow 0, \quad W \leftarrow 0$}
\STATE{Let $R = {\left(\widehat{D}\trans\widehat{D} + \rho I\right)}^{-1}$}
\COMMENT{compute efficiently by \Cref{thm:diagonal}}
\REPEAT{}
    ...
    \right) \right\}$} \COMMENT{time-domain soft-thresholding}
    \STATE{$W \leftarrow W + A - Z$} \COMMENT{update residual vector}
    \STATE{(optional) Update $\rho$ and recompute $R$}
\UNTIL{convergence}
\RETURN{$a_k$} \COMMENT{compute by \eqref{eq:complexify}}
\end{algorithmic}
\end{algorithm}
