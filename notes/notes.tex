...
\FOR{$T= 0, 1, 2, \cdots$}
    ...
    \FOR{$t = 0, 1, 2, \cdots$}
    \item $C \leftarrow \text{ridge}(X, D, \rho, DX + \rho(Z - \Omega), (1 + \rho^{-1}\Delta)^{-1})$   \COMMENT{Encode data}
    ...
    \item $SX_i \leftarrow (S^i)\trans X_{\cdot i}, \quad \sigma_i \leftarrow S_i\trans S_i$ \COMMENT{Pre-cache values}
    \FOR{$t = 0, 1, 2, \cdots$} 
    \item $F_i \leftarrow \text{ridge}(X, S_i, \rho, SX_i + \rho(F - E_i), (1 + \rho^{-1}\sigma_i)^{-1})$ \COMMENT{Optimize each point's codebook (parallel)}
